...
function calculateUndertrickScore(
  ...
  return vulnerable
    ? calculateVulnerableUndertrickScore(contract, undertricks)
    : calculateNonVulnerableUndertrickScore(contract, undertricks);
}
...
function calculateOvertrickScore(
  ...
  if (contract.doubled) {
    score +=
      overtricks * (vulnerable ? V_DOUBLED_OVERTRICK : NV_DOUBLED_OVERTRICK);
